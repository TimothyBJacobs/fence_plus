...
class Fence_Plus_Options_Controller {
	...
	private function add_fields() {
		...
		$this->options_fields[] = array(
			'field_type' => 'section_title',
			'title'      => __( 'Club Information', Fence_Plus::SLUG )
		);

		$this->options_fields['club_name'] = array(
			'slug'        => 'club_name',
			'default'     => '',
			'label'       => __( 'Club Name', Fence_Plus::SLUG ),
			'description' => '',
			'field_type'  => 'text',
			'field_args'  => array()
		);

		$this->options_fields['club_initials'] = array(
			'slug'        => 'club_initials',
			'default'     => '',
			'label'       => __( 'Club Initials', Fence_Plus::SLUG ),
			'description' => '',
			'field_type'  => 'text',
			'field_args'  => array()
		);

		$this->options_fields['club_address'] = array(
			'slug'        => 'club_address',
			'default'     => '',
			'label'       => __( 'Club Address', Fence_Plus::SLUG ),
			'description' => __( "Make sure this is the same address used on askFRED.", Fence_Plus::SLUG ),
			'field_type'  => 'text',
			'field_args'  => array()
		);
		...
		$this->options_fields['tournament_distance'] = array(
			'slug'        => 'tournament_distance',
			'default'     => 125,
			'label'       => __( 'Tournament Distance', Fence_Plus::SLUG ),
			'description' => __( "In what radius should we search for tournaments.", Fence_Plus::SLUG ),
			'field_type'  => 'number',
			'field_args'  => array(
				'min' => 1,
				'max' => 999
		...
		$this->options_fields['email_fencers_suggested'] = array(
			'slug'        => 'email_fencers_suggested',
			'default'     => false,
			'label'       => __( 'Email Fencers Suggested Tournaments', Fence_Plus::SLUG ),
			'description' => __( 'Email fencers about tournaments that have been suggested to them.', Fence_Plus::SLUG ),
			'field_type'  => 'checkbox',
			'field_args'  => array()
		);
